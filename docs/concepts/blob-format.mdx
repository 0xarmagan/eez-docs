...
`CloseBlobStream`, `Snapshot`, `Revert`, and `FinishCrossChainTransaction` are all pure brackets — their meaning comes entirely from their *position* in the stream, not from any payload:
