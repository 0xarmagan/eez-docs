---
title: "Static Entry Specification"
sidebar_label: "Static Entries"
description: "The unified reentrant table and top-level StaticExecutionEntry pool that resolve read-only cross-chain calls."
---
...
| TOP-LEVEL state-changing call that reverts | normal `ExecutionEntry` with `success == false` | entry queue | see [`EXECUTION_ENTRY_SPEC.md`](/spec/execution-entries) — out of scope here |
...
This document complements [`EXECUTION_ENTRY_SPEC.md`](/spec/execution-entries) (how `ExecutionEntry`s are built) and
[`CORE_PROTOCOL_SPEC.md`](/spec/core-protocol/) §E (rolling hash; §E.2 for the untagged static schema).
...
formula (`EEZBase.computeCrossChainCallHash` — [`CORE_PROTOCOL_SPEC.md`](/spec/core-protocol/) §C.3), with
...
lookup, and is specified in [`EXECUTION_ENTRY_SPEC.md`](/spec/execution-entries).
...
- While a batch is mid-flight (`_transientEntries.length != 0`): **only** the batch's
  ...
  [[`CAVEATS.md`](/spec/caveats)](/spec/caveats)).
...
nothing applied on L2, root unchanged ([`CORE_PROTOCOL_SPEC.md`](/spec/core-protocol/) §C, L2 prover constraints).
...
- Results fold into an **untagged** accumulator seeded at `bytes32(0)`
  ...
  key already pins the context (see [`CORE_PROTOCOL_SPEC.md`](/spec/core-protocol/) §E.2). The hash is **always**
