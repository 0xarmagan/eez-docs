...
Per-rollup operations like `addProofSystem` / `removeProofSystem`, `updateVerificationKey`, `setThreshold`, `transferOwnership`, and any owner-driven `setRoot` initiation live on the manager itself. See [`MULTI_PROVER_SPEC.md`](/spec/multi-prover) and `src/rollupContract/Rollup.sol`.
