...
```text
scan _transientLookupCalls first (meta-hook window)
then scan lookupQueue[destinationRollupId]
...
| Transient pool scan | L1 scans transient first, then persistent | L2 scans only `lookupCalls` |
